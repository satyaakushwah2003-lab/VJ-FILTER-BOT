...
async def start(client, message):
    ...
    if len(message.command) != 2:
        if PREMIUM_AND_REFERAL_MODE == True:
            buttons = [[
                ...
                InlineKeyboardButton('YouTube', url="https://youtube.com/@filmydoses"),
                InlineKeyboardButton(' Instagram ', url='https://www.instagram.com/filmy._.doses?igsh=MXdqeWw3MTZiZTkwZA==')
            ],[
                InlineKeyboardButton('Movies Channel ', url='https://t.me/Filmy_Doses'),
                ...
                InlineKeyboardButton('ᴊᴏɪɴ ᴜᴘᴅᴀᴛᴇ ᴄʜᴀɴɴᴇʟ', url='https://t.me/Filmydosesofficial')
            ]]
        else:
            buttons = [[
                InlineKeyboardButton('⤬ ᴀᴅᴅ ᴍᴇ ᴛᴏ ʏᴏᴜʀ ɢʀᴏᴜᴘ ⤬', url=f'http://t.me/{temp.U_NAME}?startgroup=true')
        ...
        if CLONE_MODE == True:
            buttons.append([InlineKeyboardButton('Request Movie', url='https://t.me/Filmydosesbot')])
        reply_markup = InlineKeyboardMarkup(buttons)
    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        try:
            if REQUEST_TO_JOIN_MODE == True:
                invite_link = await client.create_chat_invite_link(chat_id=(int('https://t.me/Filmy_Doses')), creates_join_request=True)
            else:
        ...
        try:
            ...
            if REQUEST_TO_JOIN_MODE == True:
                if TRY_AGAIN_BTN == True:
                    text = "**🕵️ Bhai Pehle BackUp Channel Join Karle Uske Bad Hi Movie Dunga**"
                else:
        ...
        for msg in msgs:
            ...
            try:
                ...
                if STREAM_MODE == True:
                    button = [[
                        InlineKeyboardButton("•Youtube •", url='https://youtube.com/@filmydoses'),
                        InlineKeyboardButton('• All Movies Channels •', url='https://t.me/Filmy_Doses')
                    ],[
        ...
        for file in files:
            ...
            if CUSTOM_FILE_CAPTION:
                try:
                    ...
                    f_caption=f_caption
            if f_caption is None:
                f_caption = f"{' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@Filmy_Doses'), files1['file_name'].split()))}"
    ...
    if CUSTOM_FILE_CAPTION:
        try:
            ...
            f_caption=f_caption
    if f_caption is None:
        f_caption = f"{' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@Filmy_Doses'), files['file_name'].split()))}"
    if not await db.has_premium_access(message.from_user.id):
        if not await check_verification(client, message.from_user.id) and VERIFY == True:
            btn = [[
                InlineKeyboardButton("ᴠᴇʀɪғʏ", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start="))
...
async def plans_cmd_handler(client, message): 
    if PREMIUM_AND_REFERAL_MODE == False:
        return 
    btn = [            
        [InlineKeyboardButton("ꜱᴇɴᴅ ᴘᴀʏᴍᴇɴᴛ ʀᴇᴄᴇɪᴘᴛ 🧾", url='https://t.me/Filmydosesbot')],
